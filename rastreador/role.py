...
def edit(id):
    ...
    if request.method == "POST":
        cargonovo = request.form["cargoNovo"][
            1:-1
        ]  # remova "" em torno do string (strip e replace não funcionaram por alguma razão)
        try:
    ...
    return render_template(
        "role/edit.html", usename=username, cargo=cargo, cargos=cargos
    )
