def test_api_veh_placa_bom(client): #podemos loca
    ...
    assert b'Teste' in response.data # Teste se localiza a ultima entrada para placa quande existirem multiplas entradas

def test_api_veh_placa_bom_2(client): #podemos loca
    response = client.get('/api/veiculo/por_placa/XYZ12C4') #tambem funciona com placas no novo formato
    assert response.status_code == 200 #conforme especificação http
    assert b'XYZ12C4' in response.data 

def test_api_veh_placa_caseinsensitivo(client): #letras podem ser maiusculas ou minusculas
    response = client.get('/api/veiculo/por_placa/aaa-1234')
